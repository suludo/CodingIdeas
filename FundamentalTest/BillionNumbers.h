...
int main(int argc, char **argv)
{
    ...
    bucket = (bucket_t*)malloc(sizeof(bucket_t)*nbuckets);
    if (bucket == NULL) exit(0);
    for (i = 0; i < nbuckets; i++) {
        bucket[i].buf = (int*)malloc(BUCKET_BUF_SIZE);
    ...
    buffer = (int*)malloc(bucket_size*4);
